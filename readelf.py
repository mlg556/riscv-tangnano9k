#!C:\Users\mirac\AppData\Local\Programs\Python\Python39\python.exe
# -------------------------------------------------------------------------------
# scripts/readelf.py
#
...
class ReadElf(object):
    ...
    def display_file_header(self):
        ...
        self._emit("  Magic:   ")
        self._emit(" ".join("%2.2x" % byte2int(b) for b in self.elffile.e_ident_raw))
        self._emitline("      ")
    ...
    def _dump_debug_info(self):
        ...
        for cu in self._dwarfinfo.iter_CUs():
            self._emitline(
                "  Compilation Unit @ offset %s:" % self._format_hex(cu.cu_offset)
            )
            self._emitline(
                "   Length:        %s (%s)"
            ...
            if cu.header.get("unit_type", False):
                ut = next(
                    (
                        key
                        for key, value in ENUM_DW_UT.items()
                        if value == cu.header.unit_type
                    ),
                    "?",
                )
                self._emitline("   Unit Type:     %s (%d)" % (ut, cu.header.unit_type))
            self._emitline(
                "   Abbrev Offset: %s" % (self._format_hex(cu["debug_abbrev_offset"]))
            ),
        ...
        for s in attr_sec.iter_subsections():
            ...
            for ss in s.iter_subsubsections():
                ...
                for attr in ss.iter_attributes():
                    self._emit("  ")
                    self._emitline(
                        describe_attr_tag_arm(attr.tag, attr.value, attr.extra)
                    )
